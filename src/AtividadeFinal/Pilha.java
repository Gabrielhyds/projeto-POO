...
public class Pilha extends EstrategiaLIFO {
	...
	public String atender() { 
	  if (!pilha.isEmpty()) { // Se a pilha não estiver vazia
	        Senha senha = pilha.getFirst(); // remove a primeira senha do topo da pilha
	        if(senha.getChamado() == true) {
	            pilha.removeFirst(); // Remove a primeira senha do topo da pilha
	            return senha.retornarSenha(); // Retorna a senha atendida
	        }
	        else {
	            return "A senha ainda não foi chamada."; // Se a senha não foi chamada
	        }
	    } 
	  return "não deu bom";
	}
